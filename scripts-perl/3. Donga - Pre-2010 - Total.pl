# ============================================================ #
# Web-scraping the total number of articles from Donga
...
my $pwd = cwd();
my ($target) = "$pwd/../output/counts/dongapre2010total.csv";
	...
	for my $year (@year_loop) {
		foreach my $month (@month_loop) {
			...
			$month_new = $month;
				...
				my $url = "http://news.donga.com/Pdf?p1=&media=16&query=.&range=1&search_date=5&sdate=".$sdate."&edate=".$edate;
				my $r= $agent->get($url);
