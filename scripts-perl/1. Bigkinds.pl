	...
	foreach $term (@term_set) {
		for my $year (@year_loop) {
			foreach my $month (@month_loop) {
				...
				my $r= $agent -> submit_form(form_name => 'topSearchForm', fields => { keyword => $term, fromDate => $fromDate, toDate => $toDate, provider_code => $paper, search_method => 2});
				...
				if ($content =~ m/<span class="count">\((\d+)건/) {
					$results = ($1);
				} else {
					$results = 0;
				}
